...
fn main() {
    ...
    let mut numbers = (0..2000001).map(|_| false).collect::<Vec<_>>();

    (0..n).for_each(|num| {
        let val: isize = reader.next();
        numbers[(val + 1000000) as usize] = true;
    });

    numbers.iter().enumerate().for_each(|(idx, &exists)| {
        if exists {
            writer.writeln(idx as isize - 1000000);
        }
    });
}
