...
fn main(){
    ...
    let word1 = r.next::<String>();
    let word2 = r.next::<String>();

    let rev_word1 = word1.iter().rev().collect::<String>();
    let rev_word2 = word2.iter().rev().collect::<String>();

    if rev_word1 > rev_word2{
        w.write(rev_word1);
    } else{
        w.write(rev_word2);
    }
