...
fn main(){
    ...
    let mut baskets = Vec::<usize>::new();
    baskets.resize_with(n, Default::default);

    (0..m).for_each(|_|{
        let st = r.next::<usize>();
        let fi = r.next::<usize>();
        let ball = r.next::<usize>();
        baskets[st - 1..fi].iter_mut().for_each(|x| *x = ball);
    });

    w.write(baskets.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(" "));
}